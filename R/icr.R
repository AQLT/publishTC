...
#'
#'
#' @param x I/C ratio [icr()] or a time series
...
x11_trend_selection <- function(x, ...){
	UseMethod("x11_trend_selection")
}
#' @rdname x11_trend_selection
#' @export
x11_trend_selection.default <- function(x, ..., freq) {
	icr <- x
	if (freq == 2) {
		length <- 5
...
#' @rdname x11_trend_selection
#' @export
x11_trend_selection.ts <- function(x, ...){
